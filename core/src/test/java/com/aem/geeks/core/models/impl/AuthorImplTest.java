...
class AuthorImplTest {
    ...
    void setUp() {
        ...
        aemContext.load().json("/com/aem/geeks/core/models/impl/Author.json",/*the left side path is load in 
        right side path which is /component we dindt store the data*/"/component");
        //the storing of data can be done in line 39,48,57,66,73... is the format of storing the data of json
    ...
    @Test
    void getFirstName() {
        aemContext.currentResource("/component/author");//here we are loading the j-son from component to author 
        //which is resource,now current resource is author which contains the j-son data
        author=aemContext.request().adaptTo(Author.class);//here we are giving or adapting the j-son date
        //to Author.class
        final String expected="AEM";//this is value which we type in the dialog
        String actual=author.getFirstName();//we are storing the j-son data in Author.class na,here
        //here it get the j-son data of which is related to 'fname' which is the object of getFirstName
        //in j-son we have fname as AEM when it matches the object fname and j-son fname it gets the value
        assertEquals(expected,actual);//finally AEM=AEM
    }
